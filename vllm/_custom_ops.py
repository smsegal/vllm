...
if hasattr(torch.ops._C, "gptq_marlin_24_gemm"):
    ...
    def _marlin_qqq_gemm_fake(
        ...
        return torch.empty(
            (size_m, size_n), dtype=torch.float16, device=a.device
        )
    ...
    def _awq_dequantize_fake(
        ...
        out_c = qout_c * 8
        return torch.empty(
            (in_c, out_c), dtype=scales.dtype, device=scales.device
        )
    ...
    def machete_prepack_B_fake(
        ...
        return torch.empty_like(
            b_q_weight, memory_format=torch.contiguous_format
        )
...
if hasattr(torch.ops._C, "ggml_dequantize"):
    ...
    def _ggml_moe_a8_fake(
        ...
        tokens = X.size(0)
        return torch.empty(
            (tokens * top_k, row), dtype=torch.float16, device=W.device
        )
...
if hasattr(torch.ops._C, "ggml_moe_a8_vec"):
    ...
    def _ggml_moe_a8_vec_fake(
        ...
        tokens = X.size(0)
        return torch.empty(
            (tokens * top_k, row), dtype=X.dtype, device=W.device
        )
...
def cutlass_scaled_mm(
    ...
    assert (
        bias is None or bias.numel() == b.shape[1] and bias.dtype == out_dtype
    )
...
def cutlass_scaled_sparse_mm(
    ...
    assert (
        bias is None
        or bias.shape[0] == bt_nzs.shape[0]
...
def allspark_repack_weight(
    ...
    scale_reorder = torch.empty(
        (1, N_32align), device=scale.device, dtype=scale.dtype
    )
...
def scaled_int8_quant(
    ...
    input_azp = (
        None if symmetric else torch.empty_like(input_scales, dtype=torch.int32)
    )
...
def wvSplitKQ(
    ...
    out = torch.empty(
        (b.shape[0], a.shape[0]), dtype=out_dtype, device=b.device
    )
...
if supports_moe_ops and hasattr(torch.ops._moe_C, "marlin_gemm_moe"):
    ...
    def marlin_gemm_moe_fake(
        ...
        return torch.empty(
            (size_m, topk, size_n), dtype=a.dtype, device=a.device
        )
...
def copy_blocks_mla(
    kv_caches: list[torch.Tensor], block_mapping: torch.Tensor
) -> None:
...
def all_reduce(
    ...
    torch.ops._C_custom_ar.all_reduce(
...
def qr_all_reduce(
    ...
    torch.ops._C_custom_ar.qr_all_reduce(
