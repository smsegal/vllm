...
class MergedReplicatedLinear(ReplicatedLinear):
    ...
    def weight_loader(
        ...
        if isinstance(param, BlockQuantScaleParameter):
            ...
            assert self.quant_method is not None
            assert isinstance(
                self.quant_method, (Fp8LinearMethod, Fp8MoEMethod)
            )
    ...
    def weight_loader(
        ...
        if is_gguf_weight_type:
            ...
            if loaded_shard_id is not None:
                ...
                param.shard_weight_type = {
                    k: loaded_weight.item() for k in idx_map
                }
            return
...
class QKVCrossParallelLinear(LinearBase):
    ...
    def q_proj_decoder(self) -> ColumnParallelLinear:
        ...
        for name, param in self.named_parameters():
            ...
            if target_param is not None:
                self.sync_weight_attrs(
                    param, target_param, mode="q_proj_decoder"
                )
        return layer
    ...
    def kv_proj_encoder(self) -> QKVParallelLinear:
        ...
        for name, param in self.named_parameters():
            ...
            if target_param is not None:
                self.sync_weight_attrs(
                    param, target_param, mode="kv_proj_encoder"
                )
        return layer
    ...
    def sync_weight_attrs(
        ...
        use_bitsandbytes_4bit = getattr(
            src_param, "use_bitsandbytes_4bit", False
        )
    ...
    def _is_same_param(
        ...
        src_param_attrs = {
            k: v
            for k, v in src_param.__dict__.items()
            if k not in key_to_ignore
        }
        map_param_attrs = {
            k: v
            for k, v in map_param.__dict__.items()
            if k not in key_to_ignore
        }
    ...
    def select_proj_params(
        ...
        target_param_list = [
            v
            for _, v in layer.named_parameters()
