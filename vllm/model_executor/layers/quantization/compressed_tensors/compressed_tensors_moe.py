    ...
    def select_gemm_impl(
        ...
        if self.use_cutlass:
            ...
            if (
                ...
                logger.debug(
                    "CutlassBatchedExpertsFp8(%s)", self.__class__.__name__
                )
...
class CompressedTensorsWNA16MarlinMoEMethod(CompressedTensorsMoEMethod):
    def __init__(
        ...
        if not (
            self.quant_config.quant_format
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        w2_scale = torch.nn.Parameter(
            torch.ones(
                num_experts, num_groups_w2, hidden_size, dtype=params_dtype
            ),
            requires_grad=False,
        )
...
class CompressedTensorsWNA16MoEMethod(CompressedTensorsMoEMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.w13_weight_packed = torch.nn.Parameter(
            layer.w13_weight_packed.transpose(1, 2)
            .contiguous()
            .view(torch.uint8),
            requires_grad=False,
        )
        layer.w2_weight_packed = torch.nn.Parameter(
            layer.w2_weight_packed.transpose(1, 2)
            .contiguous()
            .view(torch.uint8),
            requires_grad=False,
        )
