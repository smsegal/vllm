...
class SpecDecodeBaseSampler(nn.Module):
    ...
    def __init__(self, strict_mode: bool = False):
        ...
        self.request_metrics = {}  # Map request_id -> metrics dict
        self.acceptance_lengths: Optional[torch.Tensor] = None
    ...
    def init_gpu_tensors(self, device: Union[int, str]) -> None:
        ...
        self.mean_accepted_tokens = torch.tensor(
    ...
    def cleanup_request(self, request_id: str) -> None:
        """Remove metrics for a completed request."""
    ...
    def _create_output(
        ...
        request_ids: Optional[List[str]] = None,  # New parameter
    ) -> torch.Tensor:
            ...
            for i, request_id in enumerate(request_ids):
                if request_id not in self.request_metrics:
                    self.request_metrics[request_id] = {
                        ...
                        "accepted_lengths": [],  # Add this to store the lengths
                    }

                # Store this acceptance length (turns out, limits was already calculating this for us)
                self.request_metrics[request_id]["accepted_lengths"].append(
                    limits[i].item()
                )
                accepted_count = limits[i].item()
                self.request_metrics[request_id]["accepted_tokens"] += (
                    accepted_count
                )
                self.request_metrics[request_id]["draft_tokens"] += k
                self.request_metrics[request_id]["emitted_tokens"] += (
                    output_with_bonus_tokens[i] != -1
                ).sum()
                # Update mean_accepted_tokens with running average
                total_accepted = self.request_metrics[request_id][
                    "accepted_tokens"
                ]
                total_draft = self.request_metrics[request_id]["draft_tokens"]
                if total_draft > 0:
                    self.request_metrics[request_id]["mean_accepted_tokens"] = (
                        total_accepted / (total_draft / k)
                    )

        # Keep existing global metrics for backward compatibility
        # this was incorrect - the accepted mask isn't causal, so this would include tokens accepted _after_ a rejection as well.
        # self.num_accepted_tokens += accepted.sum()
        self.num_accepted_tokens += limits.sum()
    ...
    def _raise_if_incorrect_shape(
        ...
        if draft_probs is not None:
            (draft_batch_size, num_draft_probs, draft_vocab_size) = (
                draft_probs.shape
            )
