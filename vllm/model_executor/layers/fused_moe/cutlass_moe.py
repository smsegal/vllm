...
def run_cutlass_moe_fp8(
    ...
    assert (
        w1_scale.dim() == 1
        or w1_scale.size(1) == 1
        or w1_scale.shape[1] == w1.size(1)
    ), "W1 scale shape mismatch"
    assert (
        w2_scale.dim() == 1
        or w2_scale.size(1) == 1
        or w2_scale.shape[1] == w2.size(1)
    ...
    if use_batched_format:
        ...
        expert_offsets = torch.empty(
            (local_E), dtype=torch.int32, device=device
        )
        problem_sizes1 = torch.empty(
            (local_E, 3), dtype=torch.int32, device=device
        )
        ...
        num_expert = (
            global_num_experts if expert_map is None else expert_map.size(0)
        )
