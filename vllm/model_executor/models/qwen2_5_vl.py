...
class Qwen2_5_VisionMLP(nn.Module):
    def __init__(
        ...
        cls_down_proj = (
            ReplicatedLinear if use_data_parallel else RowParallelLinear
        )
        self.down_proj = cls_down_proj(
            hidden_features,
            in_features,
            bias=bias,
            quant_config=quant_config,
    ...
    def __init__(
        ...
        if use_data_parallel:
            self.qkv = ReplicatedLinear(
                embed_dim,
                self.hidden_size_per_attention_head * 3 * num_heads,
                bias=True,
                quant_config=quant_config,
                prefix=f"{prefix}.qkv",
            )
        ...
        cls_proj = ReplicatedLinear if use_data_parallel else RowParallelLinear
        self.proj = cls_proj(
