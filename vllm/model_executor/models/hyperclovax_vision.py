...
class HCXVisionMultiModalProcessor(
        ...
        if len(mm_data) > 0:
            ...
            if videos:
                ...
                _processed_outputs["pixel_values_videos"] = [
                    _processed_outputs["pixel_values_videos"][
                        sum(_num_per_videos[:_i]) : sum(
                            _num_per_videos[: _i + 1]
                        )
                    ]
                ...
                processed_outputs["input_ids"] = torch.stack(
                    [
                        replace_multimodal_token(
                            ...
                            repeats=[
                                sum(lens)
                                for lens in _processed_outputs[
    ...
    def _get_mm_fields_config(
        ...
        return dict(
            ...
            num_queries_vis_abstractors_slow_images=MultiModalFieldConfig.batched(
                "image"
            ),
            ...
            num_queries_vis_abstractors_slow_videos=MultiModalFieldConfig.batched(
                "video"
            ),
...
class HCXVisionCAbstractor(nn.Module):
    ...
    def build_net(
        ...
        self.readout = self.build_mlp(
            mlp_depth, hidden_size, output_hidden_size
        )
...
class HCXVisionForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
        ...
        **kwargs: Unpack[HCXVisionMultimodalInputs],
        ...
        if kwargs.get("pixel_values_images") is not None:
            for _pixel_values_images, _image_sizes_images in zip(
                kwargs["pixel_values_images"], kwargs["image_sizes_images"]
            ):
                ...
                if isinstance(_image_sizes_images, torch.Tensor):
                    _image_sizes_images = (
                        _image_sizes_images.detach().cpu().tolist()
                    )
        ...
        if kwargs.get("pixel_values_videos") is not None:
            for _pixel_values_videos, _vision_query_lengths_videos in zip(
                ...
                _pixel_values_videos = _pixel_values_videos.reshape(
                    sum(_len_pixel_values_videos), -1, _c, _w, _h
                ).unsqueeze(dim=0)
    ...
    def get_input_embeddings(
        ...
        **kwargs,
        ...
        if (
            kwargs.get("pixel_values_images") is not None
        ...
        if multimodal_embeddings is not None:
            ...
            if _mask_image.sum() > 0:
                inputs_embeds[_mask_image] = multimodal_embeddings[
                    : sum(_mask_image)
                ]
    ...
    def forward_images(
        ...
        concat_pixel_values_images = torch.cat(
            list(chain(*pixel_values_images)), dim=0
        )
        ...
        image_features = anyres_postprocessing(
            ...
            image_sizes=[
                image_size
                for image_sizes in image_sizes_images
    ...
    def forward_videos(
        ...
        concat_pixel_values_videos = torch.cat(
            list(chain(*pixel_values_videos)), dim=0
        )
