...
if current_platform.is_rocm():
    ...
    def _vllm_layout_trans_kernel(
        ...
        batch_query_indexes = tl.load(
            b_query_lens_loc + batch_idx + tl.arange(0, 2)
        )
        ...
        batch_token_indexes = tl.load(
            b_seq_lens_loc + batch_idx + tl.arange(0, 2)
        )
        ...
        if block_idx * BLOCK_SIZE < seq_len:
            ...
            k_vals = tl.load(
                k_buffer_ptr + kv_buffer_off, mask=block_mask, other=0.0
            )
            if k_vals.dtype.is_fp8():
                k_vals = (k_vals.to(tl.float32) * tl.load(k_scale)).to(
                    output_dtype
                )
            else:
            ...
            v_vals = tl.load(
                v_buffer_ptr + kv_buffer_off, mask=block_mask, other=0.0
            )
            if v_vals.dtype.is_fp8():
                v_vals = (v_vals.to(tl.float32) * tl.load(v_scale)).to(
                    output_dtype
                )
            else:
...
class AiterFlashAttentionMetadataBuilder(
    ...
    def build(
        ...
        if max_query_len > 1:
            ...
            torch.cumsum(
                seq_lens, dim=0, dtype=cu_seq_lens.dtype, out=cu_seq_lens[1:]
            )
