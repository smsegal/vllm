...
class LLMEngine:
    ...
    def validate_output(
        ...
        if (TYPE_CHECKING or do_validate) and not isinstance(
            output, output_type
        ):
            raise TypeError(
    ...
    def __init__(
        ...
        self.decoding_config = (
            vllm_config.decoding_config
            or DecodingConfig(  # noqa
            )
        )
        self.observability_config = (
            vllm_config.observability_config
            or ObservabilityConfig(  # noqa
            )
        )
        ...
        if self.model_config.use_async_output_proc:
            ...
            self.async_callbacks = [
                partial(
                    process_model_outputs, ctx=self.scheduler_contexts[v_id]
                )
    ...
    def _add_processed_request(
        ...
        seq = Sequence(
            seq_id, decoder_inputs, block_size, eos_token_id, lora_request
        )
    ...
    def _create_sequence_group_with_sampling(
        ...
        if (
            sampling_params.logprobs and sampling_params.logprobs > max_logprobs
        ) or (
            sampling_params.prompt_logprobs
            and sampling_params.prompt_logprobs > max_logprobs
        ):
            raise ValueError(
                f"Cannot request more than {max_logprobs} logprobs."
            )
    ...
    def has_unfinished_requests(self) -> bool:
        ...
        return any(
            scheduler.has_unfinished_seqs() for scheduler in self.scheduler
        )
    ...
    def _process_model_outputs(
        ...
        acceptance_lengths_list: Optional[list[int]] = None
        if outputs and isinstance(outputs[0], SamplerOutput):
            acceptance_lengths_list = outputs[0].acceptance_lengths
        ...
        for i in indices:
            ...
            if acceptance_lengths_list is not None:
                if seq_group.acceptance_lengths is None:
                    seq_group.acceptance_lengths = []
                seq_group.acceptance_lengths.append(acceptance_lengths_list[i])
    ...
    def _advance_to_next_step(
        ...
        for (
            ...
            scheduled_seq_group,
        ) in zip(seq_group_metadata_list, output, scheduled_seq_groups):
            seq_group = scheduled_seq_group.seq_group
            ...
            if seq_group_metadata.do_sample:
                ...
                seq.append_token_id(
                    sample.output_token, sample.logprobs, sample.output_embed
                )
    ...
    def step(self) -> List[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        if (
            not self._has_remaining_steps(seq_group_metadata_list)
    ...
    def _validate_model_inputs(
        ...
        if encoder_inputs is not None:
            self._validate_model_input(
                encoder_inputs, lora_request, prompt_type="encoder"
            )

        self._validate_model_input(
            decoder_inputs, lora_request, prompt_type="decoder"
        )
