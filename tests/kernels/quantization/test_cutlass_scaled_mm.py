...
def test_cutlass_fp8_group_gemm(
    ...
    for g in range(num_experts):
        ...
        baseline_g = baseline_scaled_mm(
            a_g, b_g, scale_a, scale_b, out_dtype, None
        )
        baseline_tensors.append(baseline_g)

    a_tensors_stacked = torch.empty(
        (expert_offsets[num_experts], k_g),
        device=device,
    ...
    for g in range(num_experts):
        a_tensors_stacked[expert_offsets[g] : expert_offsets[g + 1]] = (
            a_tensors[g]
        )
    ...
    if per_act_token:
        ...
        for g in range(num_experts):
            a_scales_tensors_stacked[
                expert_offsets[g] : expert_offsets[g + 1]
            ] = a_scales_tensors[g]
    else:
