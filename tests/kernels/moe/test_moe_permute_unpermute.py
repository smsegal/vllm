...
def torch_permute(
    ...
    if expert_map is not None:
        ...
        topk_ids = is_local_expert * (
            topk_ids - start_expert
        ) + not_local_expert * (topk_ids + n_expert)
    ...
    if align_block_size is None:
        permuted_hidden_states = hidden_states[
        ...
        dst_row_id2src_row_id_map[expert_first_token_offset[-1] :] = (
            n_token * topk
        )
        return [
        ...
        for i in range(1, n_local_expert + 1):
            ...
            permuted_hidden_states[
                align_first_token_offset : align_first_token_offset
                + n_token_in_expert,
                ...,
        ...
        for i in range(n_token * topk):
            ...
            if eid >= n_local_expert:
                ...
                align_src_row_id2dst_row_id[i] = (
                    align_expert_first_token_offset[-1]
                )
                continue
            ...
            align_src_row_id2dst_row_id[i] = (
                align_first_token_offset + token_offset
            )
...
def torch_unpermute(
    ...
    mask = torch.zeros(
        permuted_hidden_states.shape[0], dtype=bool, device="cuda"
    )
