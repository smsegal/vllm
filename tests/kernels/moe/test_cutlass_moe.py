...
class MOETensors8Bit(MOETensors):
    ...
    def make_moe_tensors_8bit(
        ...
        w1_scale = torch.empty(
            (e, n_b_scales, 1), device="cuda", dtype=torch.float32
        )
        w2_scale = torch.empty(
            (e, k_b_scales, 1), device="cuda", dtype=torch.float32
        )
    ...
    def slice_experts():
        ...
        for i in range(0, num_experts, num_local_experts):
            ...
            expert_map = torch.tensor(
                expert_map, dtype=torch.int32, device="cuda"
            )
...
def test_cutlass_moe_8_bit_no_graph(
    ...
    with set_current_vllm_config(vllm_config):
        mt = MOETensors8Bit.make_moe_tensors_8bit(
            m, k, n, e, per_act_token, per_out_ch
        )

        score = torch.randn((m, e), device="cuda", dtype=torch.half)
        topk_weights, topk_ids, _ = fused_topk(
            mt.a, score, topk, renormalize=False
        )

        # Note that we are using the dequantized versions of the tensors.
        # Using a, w1 and w2 directly results in minor output differences.
        triton_output = fused_experts(
            mt.a_d, mt.w1_d, mt.w2_d, topk_weights, topk_ids
        )
...
def test_cutlass_moe_8_bit_cuda_graph(
    ...
    with set_current_vllm_config(vllm_config):
        ...
        with torch.cuda.graph(graph, stream=stream):
            cutlass_output = run_8_bit(
                mt, topk_weights, topk_ids, per_act_token
            )
        ...
        torch.testing.assert_close(
            triton_output, cutlass_output, atol=9e-2, rtol=1e-2
        )
